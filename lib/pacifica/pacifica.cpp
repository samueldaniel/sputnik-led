...
void pacifica_loop(CRGB **leds, int num_strips, int num_leds) {
  EVERY_N_MILLISECONDS(20) {
    ...
    for (int s = 0; s < num_strips; s++) {
      fill_solid(leds[s], num_leds, CRGB(2, 6, 10));
    }

    // Render each of four layers, with different scales and speeds, that vary
    // over time
    pacifica_one_layer(leds, num_strips, num_leds, pacifica_palette_1,
                       sCIStart1, beatsin16(3, 11 * 256, 14 * 256),
                       beatsin8(10, 70, 130), 0 - beat16(301));
    pacifica_one_layer(leds, num_strips, num_leds, pacifica_palette_2,
                       sCIStart2, beatsin16(4, 6 * 256, 9 * 256),
                       beatsin8(17, 40, 80), beat16(401));
    pacifica_one_layer(leds, num_strips, num_leds, pacifica_palette_3,
                       sCIStart3, 6 * 256, beatsin8(9, 10, 38),
                       0 - beat16(503));
    pacifica_one_layer(leds, num_strips, num_leds, pacifica_palette_3,
                       sCIStart4, 5 * 256, beatsin8(8, 10, 28), beat16(601));

    // Add brighter 'whitecaps' where the waves lines up more
    pacifica_add_whitecaps(leds, num_strips, num_leds);

    // Deepen the blues and greens a bit
    pacifica_deepen_colors(leds, num_strips, num_leds);
    FastLED.show();
  }
}
...
void pacifica_add_whitecaps(CRGB **leds, int num_strips, int num_leds) {
  ...
  for (int s = 0; s < num_strips; s++) {
    for (uint16_t i = 0; i < num_leds; i++) {
      ...
      wave += 7;
      uint8_t l = leds[s][i].getAverageLight();
      if (l > threshold) {
        uint8_t overage = l - threshold;
        uint8_t overage2 = qadd8(overage, overage);
        leds[s][i] += CRGB(overage, overage2, qadd8(overage2, overage2));
      }
    }
  }
}
...
void pacifica_deepen_colors(CRGB **leds, int num_strips, int num_leds) {
  for (int s = 0; s < num_strips; s++) {
    for (uint16_t i = 0; i < num_leds; i++) {
      ...
      leds[s][i].green = scale8(leds[s][i].green, 200);
      leds[s][i] |= CRGB(2, 5, 7);
    }
  }
}
